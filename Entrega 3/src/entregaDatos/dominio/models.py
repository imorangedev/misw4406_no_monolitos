...
def message_factory(message: Dict[str, Any]) -> Optional[BaseMessage]:
    ...
    if not isinstance(message, dict):
        logger.error(
            f"Se esperaba un diccionario, pero se recibió: {type(message).__name__}.")
        return None

    if type_message_key not in message:
        logger.error(
            f"Falta la clave '{type_message_key}'. Mensaje recibido: {message}")
        return None

    try:
        message[type_message_key] = MessageType(
            message[type_message_key]).value
    except ValueError:
        logger.error(
            f"Tipo de mensaje desconocido: {message.get(type_message_key, 'N/A')}")
        return None
