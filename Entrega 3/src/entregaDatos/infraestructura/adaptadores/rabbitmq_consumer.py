...
class RabbitMQConsumer(MessageConsumerPort):
    ...
    def _create_rabbitmq_channel(self):
        try:
            ...
            self.logger.error(f"Error al conectar con RabbitMQ: {e}", exc_info=True)
            raise
    ...
    def _callback(self, ch, method, properties, body):
        try:
            ...
            self.logger.error(f"Mensaje JSON inválido, no se pudo decodificar: {body}")
            ch.basic_ack(delivery_tag=method.delivery_tag)  
            
        except Exception as e:
            self.logger.error(f"Error procesando mensaje: {e} | Datos recibidos: {body}", exc_info=True)
    ...
    def consume_messages(self):
        while True:
            try:
                ...
                self.logger.info(f"Worker en ejecución | Escuchando mensajes en la cola [{self.queue_name}]")
                channel.start_consuming()

            except Exception as e:
                self.logger.error(f"Error en worker: {e} | Reintentando en 5s...", exc_info=True)
                time.sleep(5)
